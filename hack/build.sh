...
LDFLAGS="${LDFLAGS} -X github.com/openshift-splat-team/ptr-record-operator/pkg/version.Raw=${GIT_TAG} -X github.com/openshift-splat-team/ptr-record-operator/pkg/version.Commit=${GIT_COMMIT}"
TAGS="${TAGS:-}"
OUTPUT="${OUTPUT:-bin/ptr-record-operator}"
...
go build "${GOFLAGS}" -ldflags "${LDFLAGS}" -tags "${TAGS}" -o "${OUTPUT}" main.go